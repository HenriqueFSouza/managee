    ...
    const fetchEvent = async () => {
      try {
        ...
        Object.keys(event).forEach((key) => {
          setValue(key as keyof EventFormData, event[key]);
        });
        setLoading(false);
      } catch (error) {
        toast.error('Não encontramos este evento.');
      }
    };
    fetchEvent();
  }, [id, setValue]);
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              className={`mt-1 block w-full rounded-md border-gray-300 border-[1px]  p-2 shadow-sm outline-none focus:border-accent focus:ring-2 focus:ring-accent focus:ring-opacity-50 ${errors.description ? 'border-red-500' : ''}`}
              rows={3} />
